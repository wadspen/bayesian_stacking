...
\spencerr{The following was added:
...
\item On page 10, I have some questions about Theorem 1 and its proof.
  ...
  \spencerq{We didn't include a result for the log score for teh reason you gave
    about not finding a general upper bounding function but also because we 
    focus more on the CRPS in this paper. What we need to bound to get similar
    results for the log score is the linear pool density function $\bar{p}(y)$. 
    ...
    \[
     m(y) = 
      \begin{cases*}
        \text{log}(M)  & if \(p(y) \ge 1\) \\
        -\text{log}(\epsilon)   & if \(p(y) < 1\)
      \end{cases*}
    \]
...
\spencerr{The paragraph below was added to the supplmentary materials. It is 
...
that below. See also responses to (1) and (4) of AE1.
